...
public class ConsistentHashLoadBalancer extends AbstractLoadBalance
{
    ...
    static class ConsistentHashSelector {
        ...
        ConsistentHashSelector(List<URL> invokers, int replicaNumber, int identityHashCode) {
        ...
        public URL select(String rpcServiceKey) {
            byte[] digest = md5(rpcServiceKey);
            return selectForKey(hash(digest, 0));
        }

        public URL selectForKey(long hashCode) {
            Map.Entry<Long, URL> entry = virtualInvokers.tailMap(hashCode, true).firstEntry();
