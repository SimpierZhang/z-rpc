...
public class URLParser {
            ...
            for (String part : parts) {
                ...
                if (part.length() > 0) {
                    ...
                    if (j >= 0) {
                        ...
                        if (key.startsWith(URLKeyConstants.DEFAULT_PREFIX)) {
                            parameters.putIfAbsent(key.substring(URLKeyConstants.DEFAULT_PREFIX.length()), value);
                        }
                    } else {
    ...
    public static URL buildPathToUrl(String serviceName){
        return URL.builder().protocol(URLKeyConstants.Z_RPC_PROTOCOL).path(serviceName).build();
    }
